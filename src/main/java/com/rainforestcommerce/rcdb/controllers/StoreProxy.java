...
public class StoreProxy {
	...
	public static ArrayList<Shipment> getShipmentsForStore(Store store){
	    ...
	    try {
            Connection conn = ConnectionProxy.connect();
            PreparedStatement statement = conn.prepareStatement("SELECT * FROM shipments INNER JOIN Stores ON Shipments.store_id = Stores.store_id WHERE store_id = ?");
            statement.setString(1, Long.toString(store.getStoreId()));
            ResultSet rs = statement.executeQuery();
            while (rs.next()) {
	...
	public static ArrayList<Product> searchProducts(Store store, String search){ //Function will change once store inventory is added
	    ...
	    try {
            Connection conn = ConnectionProxy.connect();
            PreparedStatement statement = conn.prepareStatement("SELECT * FROM products WHERE brand_name LIKE '%'+search+'%') > 0");
            //statement.setString(1, Long.toString(store.getStoreId()));
    ...
    public static String getStoreNameByStoreId(long storeId){
        String storeName = "";
        try{
            Connection conn = ConnectionProxy.connect();
            PreparedStatement ps = conn.prepareStatement("SELECT store_name FROM stores WHERE store_id=?;");
            ps.setString(1, Long.toString(storeId));
            ResultSet rs = ps.executeQuery();
            storeName = rs.getString("store_name");
            conn.close();
        }catch(SQLException sqle){
            LOGGER.warning("could not getStoreNameById for storeId: "+storeId);
        }
        return storeName;
    }
